## pSEO Quality Report

### Summary
- Total pages checked: 55
- Issues found: 35 (5 critical, 29 warnings, 1 informational)

### Thin Content
- **CRITICAL** `/learn-finnish/a1-to-c-level` Thin content: < 300 words of page-owned editorial text (words: 113, uniqueWords: 84)
- **Warning** `/resources/textbooks-and-grammar` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 250, uniqueWords: 121)
- **Warning** `/resources/apps-and-digital-tools` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 262, uniqueWords: 150)
- **Warning** `/resources/tv-and-video` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 212, uniqueWords: 110)
- **Warning** `/resources/radio-and-podcasts` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 234, uniqueWords: 115)
- **CRITICAL** `/resources/music` Extremely thin list page: < 200 words of unique content (words: 148, uniqueWords: 82)
- **CRITICAL** `/resources/community-and-social` Extremely thin list page: < 200 words of unique content (words: 124, uniqueWords: 70)
- **CRITICAL** `/resources/courses-and-institutions` Extremely thin list page: < 200 words of unique content (words: 187, uniqueWords: 110)
- **CRITICAL** `/resources/exams-and-certifications` Extremely thin list page: < 200 words of unique content (words: 109, uniqueWords: 70)

### Duplicate Content
- None

### Keyword Cannibalization
- **Warning** Potential keyword/intent overlap between tag hubs (shared episode roster) (a: /learn-finnish/a1-to-c-level, b: /learn-finnish/immersion-method, sharedEpisodes: 9, overlap: 0.75, aLabel: From A1 to C Level Finnish, bLabel: Learning Finnish Through Immersion)

### Metadata Issues
- **Warning** `/episode/13` Title length outside 30–70 chars (length: 88)
- **Warning** `/episode/12` Title length outside 30–70 chars (length: 87)
- **Warning** `/episode/11` Title length outside 30–70 chars (length: 93)
- **Warning** `/episode/7` Title length outside 30–70 chars (length: 79)
- **Warning** `/episode/3` Title length outside 30–70 chars (length: 79)
- **Warning** `/blog/how-to-learn-finnish-english-only-thursdays` Meta description length outside 100–170 chars (length: 210)
- **Warning** `/blog/not-fluent-after-yki` Meta description length outside 100–170 chars (length: 195)
- **Warning** `/blog/is-yki-easiest-path-finnish-citizenship` Meta description length outside 100–170 chars (length: 231)
- **Warning** `/blog/finnish-language-requirements-for-citizenship` Title length outside 30–70 chars (length: 78)
- **Warning** `/blog/finnish-language-requirements-for-citizenship` Meta description length outside 100–170 chars (length: 194)
- **Warning** `/blog/immigrants-pay-more-taxes-than-they-receive-benefits` Title length outside 30–70 chars (length: 75)
- **Warning** `/blog/immigrants-pay-more-taxes-than-they-receive-benefits` Meta description length outside 100–170 chars (length: 63)
- **Warning** `/blog/deborah-laajanen-finnish-learning-journey` Meta description length outside 100–170 chars (length: 239)
- **Warning** `/blog/best-finnish-podcasts-for-learning-the-language` Meta description length outside 100–170 chars (length: 200)
- **Warning** `/blog/why-i-started-a-podcast-about-language-learning` Title length outside 30–70 chars (length: 96)
- **Warning** `/blog/why-i-started-a-podcast-about-language-learning` Meta description length outside 100–170 chars (length: 188)
- **Warning** `/guests/hamed` Title length outside 30–70 chars (length: 29)
- **Warning** `/guests/emily` Title length outside 30–70 chars (length: 29)
- **Warning** `/learn-finnish/without-a-finnish-speaking-partner` Title length outside 30–70 chars (length: 72)
- **Warning** `/resources/textbooks-and-grammar` Title length outside 30–70 chars (length: 86)
- **Warning** `/resources/tv-and-video` Title length outside 30–70 chars (length: 81)
- **Warning** `/resources/reading` Title length outside 30–70 chars (length: 81)

### Linking Issues
- **Warning** `/episode/[id]` Episode tags are not internal links to `/learn-finnish/[tag-slug]` (rendered as display-only pills)
- **Warning** `/episode/[id]` `Resources mentioned` are rendered as plain text; consider linking to related `/resources/[category-slug]` where possible

### Scaled Content Abuse Risk
- **Info** Filler intro patterns detected (possible low-value preamble) (pages: 1)

### Heading Hierarchy
- None

### Robots and Indexation
- `robots.txt` present ✅
- Sitemap: generated via `src/app/sitemap.ts`

### Pruning Recommendations
- Pages to remove: 0
- Pages to merge: 0
- Pages to noindex: 0
- Pages to enrich: 9
  - `/learn-finnish/a1-to-c-level`
  - `/resources/apps-and-digital-tools`
  - `/resources/community-and-social`
  - `/resources/courses-and-institutions`
  - `/resources/exams-and-certifications`
  - `/resources/music`
  - `/resources/radio-and-podcasts`
  - `/resources/textbooks-and-grammar`
  - `/resources/tv-and-video`

### Action Required
1. **CRITICAL** `/learn-finnish/a1-to-c-level` Thin content: < 300 words of page-owned editorial text (words: 113, uniqueWords: 84)
2. **CRITICAL** `/resources/music` Extremely thin list page: < 200 words of unique content (words: 148, uniqueWords: 82)
3. **CRITICAL** `/resources/community-and-social` Extremely thin list page: < 200 words of unique content (words: 124, uniqueWords: 70)
4. **CRITICAL** `/resources/courses-and-institutions` Extremely thin list page: < 200 words of unique content (words: 187, uniqueWords: 110)
5. **CRITICAL** `/resources/exams-and-certifications` Extremely thin list page: < 200 words of unique content (words: 109, uniqueWords: 70)
6. **Warning** `/resources/textbooks-and-grammar` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 250, uniqueWords: 121)
7. **Warning** `/resources/apps-and-digital-tools` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 262, uniqueWords: 150)
8. **Warning** `/resources/tv-and-video` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 212, uniqueWords: 110)
9. **Warning** `/resources/radio-and-podcasts` Potentially thin list page: < 300 words of content (consider longer intro or more items) (words: 234, uniqueWords: 115)
10. **Warning** Potential keyword/intent overlap between tag hubs (shared episode roster) (a: /learn-finnish/a1-to-c-level, b: /learn-finnish/immersion-method, sharedEpisodes: 9, overlap: 0.75, aLabel: From A1 to C Level Finnish, bLabel: Learning Finnish Through Immersion)
11. **Warning** `/episode/13` Title length outside 30–70 chars (length: 88)
12. **Warning** `/episode/12` Title length outside 30–70 chars (length: 87)
13. **Warning** `/episode/11` Title length outside 30–70 chars (length: 93)
14. **Warning** `/episode/7` Title length outside 30–70 chars (length: 79)
15. **Warning** `/episode/3` Title length outside 30–70 chars (length: 79)
16. **Warning** `/blog/how-to-learn-finnish-english-only-thursdays` Meta description length outside 100–170 chars (length: 210)
17. **Warning** `/blog/not-fluent-after-yki` Meta description length outside 100–170 chars (length: 195)
18. **Warning** `/blog/is-yki-easiest-path-finnish-citizenship` Meta description length outside 100–170 chars (length: 231)
19. **Warning** `/blog/finnish-language-requirements-for-citizenship` Title length outside 30–70 chars (length: 78)
20. **Warning** `/blog/finnish-language-requirements-for-citizenship` Meta description length outside 100–170 chars (length: 194)
21. **Warning** `/blog/immigrants-pay-more-taxes-than-they-receive-benefits` Title length outside 30–70 chars (length: 75)
22. **Warning** `/blog/immigrants-pay-more-taxes-than-they-receive-benefits` Meta description length outside 100–170 chars (length: 63)
23. **Warning** `/blog/deborah-laajanen-finnish-learning-journey` Meta description length outside 100–170 chars (length: 239)
24. **Warning** `/blog/best-finnish-podcasts-for-learning-the-language` Meta description length outside 100–170 chars (length: 200)
25. **Warning** `/blog/why-i-started-a-podcast-about-language-learning` Title length outside 30–70 chars (length: 96)
26. **Warning** `/blog/why-i-started-a-podcast-about-language-learning` Meta description length outside 100–170 chars (length: 188)
27. **Warning** `/guests/hamed` Title length outside 30–70 chars (length: 29)
28. **Warning** `/guests/emily` Title length outside 30–70 chars (length: 29)
29. **Warning** `/learn-finnish/without-a-finnish-speaking-partner` Title length outside 30–70 chars (length: 72)
30. **Warning** `/resources/textbooks-and-grammar` Title length outside 30–70 chars (length: 86)
